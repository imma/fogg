...
resource "aws_route" "service_peering" {
  route_table_id            = "${element(aws_route_table.service.*.id,count.index)}"
  destination_cidr_block    = "${lookup(data.terraform_remote_state.global.org,"peering_${data.terraform_remote_state.env.env_name}_cidr_${element(split(" ",lookup(data.terraform_remote_state.global.org,"peering_${data.terraform_remote_state.env.env_name}")),count.index)}")}"
  vpc_peering_connection_id = "${lookup(data.terraform_remote_state.global.org,"peering_${data.terraform_remote_state.env.env_name}_pcx_${element(split(" ",lookup(data.terraform_remote_state.global.org,"peering_${data.terraform_remote_state.env.env_name}")),count.index)}")}"
